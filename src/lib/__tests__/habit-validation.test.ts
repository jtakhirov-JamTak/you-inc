...
// endpoint boundary — pin the bounds (partial patch, clearable area, term/weekday
// limits, bad UUIDs) so a loosened schema can't let a malformed update through.
// (Cross-field validity — recurrence only for weekly, term only for assets — is
// enforced in the handler against the fetched habit, not here.)
...
describe("updateHabitSchema", () => {
  it("accepts a partial patch (any subset of editable fields)", () => {
    ...
    expect(
      updateHabitSchema.safeParse({
        habitId: UUID,
        recurrence: { type: "weekdays", days: [1, 4, 6] },
      }).success,
    ).toBe(true);
  });
  ...
  it("rejects a non-uuid id, an out-of-set term, empty title, and a bad weekday", () => {
    ...
    expect(
      updateHabitSchema.safeParse({ habitId: UUID, recurrence: { type: "weekdays", days: [7] } })
        .success,
    ).toBe(false);
  });
});
