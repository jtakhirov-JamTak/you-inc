...
- **A `SCORING_VERSION` bump needs a CODIFIED ledger reset (or a runtime version guard) — a comment promising a hand-run TRUNCATE is not a reset.** The 2026-06-29 v2→v3 cutover's migrations *say* "the one-time clean-reset (TRUNCATE …)" but no migration performs it: `0022`'s new cadence CHECK forces a `habits` wipe, yet `price_ledger` FKs to `auth.users` (not `habits`), so stale v2 `habit_week` rows survive and the idempotent-by-key upsert silently drops the v3 recomputation — permanently mixing two scoring regimes in one user's value. Fix: `settleUser` now reads the ledger for any habit-settlement row with `scoring_version < SCORING_VERSION` and THROWS `settlement_version_mismatch` (fail-loud → Home "value unavailable") rather than booking a mixed regime; sprint rows are excluded (their payoff math is version-stable). Lesson: when scoring math changes, codify the reset in the same migration series **or** guard the booking path — never rely on a hand-run cleanup for an irreversible store.
